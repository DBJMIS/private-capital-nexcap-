...
export async function POST(req: Request, ctx: Ctx) {
  ...
  const submittedDateOpt = form.get('submitted_date');
  ...
  if (!ALLOWED_MIME_TYPES.has(file.type) && !hasAllowedExt) {
    return NextResponse.json({ error: 'Only PDF and Word documents (.docx) are supported' }, { status: 400 });
  }
  ...
  if (uErr || !updated) {
    return NextResponse.json({ error: uErr?.message ?? 'Failed to update obligation' }, { status: 500 });
  }
